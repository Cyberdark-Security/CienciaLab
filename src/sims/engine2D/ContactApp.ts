...
constructor(elem_ids: ElementIDs) {
  ...
  //this.sim.setExtraAccel(ExtraAccel.VELOCITY);
  ...
  let pn = new ParameterNumber(this, ContactApp.en.NUM_BODIES,
...
setMass1(value: number): void {
  if (this.mass1 != value) {
    this.mass1 = value;
    const body1 = this.sim.getBody(ContactApp.en.BLOCK+'1');
    body1.setMass(value);
...
static readonly i18n = Util.LOCALE === 'de' ? ContactApp.de_strings : Util.LOCALE === 'es' ? ContactApp.es_strings : ContactApp.en;
...
Util.defineGlobal('sims$engine2D$ContactApp', ContactApp);
